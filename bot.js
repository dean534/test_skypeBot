    ...
    async onTurn(turnContext) {
        ...
        if (turnContext.activity.type === ActivityTypes.Message) {
            ...
            case '維護時間':
                ...
                【維護公告】
                - 維護時間：12/28(四) 10:00 ~ 12:00 (2小時)
                - 維護內容：修正Bug問題及改善角色卡頓現象（請參照後續公告）
                
                ※ 詳細請參照後續更新內容公告
                ※ 請留意，在維護時無法登入遊戲
                ※ 維護結束後務必在Steam進行遊戲更新
                
                維護結束後為了讓大家體驗更舒適的遊戲環境我們PUBG一定會全力以赴。請敬待。
                
                感謝大家的支持及耐心等待～`);
                break;
            case 'api 文件':
                reply = { type: ActivityTypes.Message };
                reply.attachments = [this.getInternetAttachment('api document', 'pdf', 'http://www.pdf995.com/samples/pdf.pdf')];
            ...
            case '登入流程':
                const buttons = [
                    { type: ActionTypes.ImBack, title: '普通登入', value: '普通登入' },
                    { type: ActionTypes.ImBack, title: 'Token 登入', value: 'Token 登入' },
                    { type: ActionTypes.ImBack, title: 'appLink 登入', value: 'appLink 登入' }
                ];
                // construct hero card.
            ...
            for (let idx in turnContext.activity.membersAdded) {
                if (turnContext.activity.membersAdded[idx].id !== turnContext.activity.recipient.id) {
                    ...
                    const buttons = [
                        ...
                        { type: ActionTypes.ImBack, title: 'api 文件', value: 'api 文件' },
                        { type: ActionTypes.ImBack, title: '登入流程', value: '登入流程' }
                    ];
                    // construct hero card.
                    const card = CardFactory.heroCard('', undefined, buttons, { text: `
                    請問您有什麼疑問？
